...
For logical, since we divide out input into two categories, greater than or smaller than 0, so for input that greater than 0 has output TRUE counts are 1, and for input that smaller than 0 has output FALSE counts as 0. For factor, there are 3 different factor levels, which are welldone, good, and bad. So the computer can assign numbers 1 to 3 to bad, good and welldone accordingly. 
